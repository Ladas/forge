...
pub fn parse_duration(input: &str) -> Result<Duration, ForgeError> {
    if let Some(ms) = input.strip_suffix("ms") {
        ...
        return bounded(Duration::from_millis(val), input);
    }
    if let Some(secs) = input.strip_suffix('s') {
        let val: u64 = secs
            .parse()
            .map_err(|err| ForgeError::Config(format!("invalid duration '{input}': {err}")))?;
        return bounded(Duration::from_secs(val), input);
    }
...
mod bound_tests {
    ...
    fn parse_duration_rejects_values_that_would_overflow_instant() {
        ...
        assert!(result.is_err(), "u64::MAX seconds must be rejected, not returned");
    }
